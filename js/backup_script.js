...
window.addEventListener('click', function(event) {
	...
	if (event.target.className === 'save_report'){
		...
		// console.log(name, input_report.value, id)
		save_report(event.path[1])
...
function save_report(event) {
	...
	var id = event.querySelector('.id_user_vk').innerText;
	var name = event.querySelector('.name_report').innerText;
	var report = event.querySelector('#r2').value;
	...
	var a = localStorage.setItem(id, rep);
